...
class Analytics:
    ...
    def get_rows(self):
        ...
        for entity, period_data in self.entity_periodic_data.items():
            ...
            if self.filters.tree_type == "Item":
                ...
                row["available_qty"] = period_data.get("available_qty")
    ...
    def get_periodic_data(self):
        ...
        for d in self.entries:
            ...
            if self.filters.tree_type == "Item":
                ...
                self.entity_periodic_data[d.entity]["available_qty"] = self.get_available_qty(d.entity)


    def get_period(self, posting_date):
        if self.filters.range == "Daily":
            period = str(posting_date.strftime("%d-%m-%Y"))
    ...
    def get_period_date_ranges(self):
        ...
        increment = {"Daily": 1, "Monthly": 1, "Quarterly": 3, "Half-Yearly": 6, "Yearly": 12}.get(self.filters.range, 1)

        if self.filters.range == "Daily":
            # Generate all dates between from_date and to_date for Daily range
            current_date = from_date
            self.periodic_daterange = []
            while current_date <= to_date:
                self.periodic_daterange.append(current_date)
                current_date = add_days(current_date, 1)  # Increment by one day using add_days
        ...
        if self.filters.range == "Daily":
            # Generate all dates from from_date to to_date for "Daily" range
            current_date = from_date
            while current_date <= to_date:
                self.periodic_daterange.append(current_date)
                current_date = add_days(current_date, 1)  # Increment by one day
    ...
    def get_available_qty(self, item_code):
        # Initialize warehouse filter condition
        warehouse_filter = ""
        values = [item_code]

        if self.filters.warehouse:
            warehouse_filter = "and warehouse = %s"
            values.append(self.filters.warehouse)

        # Construct the SQL query
        query = f"""
            select sum(actual_qty) as actual_qty 
            from `tabBin` 
            where item_code = %s {warehouse_filter}
        """
        # Execute the query
        bin = frappe.db.sql(query, tuple(values), as_dict=1)

        # Return the available quantity
        return bin and flt(bin[0].get("actual_qty", 0)) or 0
