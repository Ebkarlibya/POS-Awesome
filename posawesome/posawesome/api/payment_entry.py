...
def get_outstanding_invoices(
    customer=None, company=None, currency=None, pos_profile=None, invoice_id=None
):
    try:
...
def process_pos_payment(payload):
    ...
    if len(data.selected_invoices) > 0 and data.total_selected_invoices > 0:
        ...
        for invoice in data.selected_invoices:
            ...
            if "outstanding_amount" not in invoice or not invoice.get(
                "outstanding_amount"
            ):
                frappe.log_error(
                ...
                try:
                    ...
                    frappe.log_error(
                        f"Error fetching invoice details: {str(e)}", "POS Payment Error"
                    )
                    errors.append(
            ...
            if created_payment and new_payment_entry:
                ...
                try:
                    ...
                    if not mode_of_payment:
                        ...
                        default_cash_mop = data.pos_profile.get(
                    ...
                    if not mode_of_payment:
                        mode_of_payment = (
                            "Cash"  # Default to Cash if nothing else available
                        )
                ...
                try:
                    ...
                    for invoice in data.selected_invoices:
                        ...
                        allocated_invoices.append(
                            {"name": invoice_name, "amount": allocation}
                        )
                    ...
                    if (
                        not created_payment
                        or payment_entry.name != new_payment_entry.name
                    ):
                        frappe.log_error(
                    ...
                    if (
                        ...
                        and new_payment_entry.docstatus == 0
                    ):
                        try:
                    ...
                    frappe.log_error(
                        f"Error allocating payment: {str(e)}", "POS Payment Error"
                    )
    ...
    try:
        ...
        if len(je.accounts) > 1:  # Need at least 2 entries (bank + receivable)
            ...
            if abs(total_debit - total_credit) > 0.01:
                ...
                frappe.log_error(
                    "Added adjustment entry to balance JE", "Direct JE Debug"
                )
...
def setup_payment_entry_cancel_hook():
    ...
    try:
        ...
        if not custom_field_exists:
            ...
            frappe.log_error(
                "Custom field 'posa_linked_je' already exists for Payment Entry",
                "POS Setup",
            )
